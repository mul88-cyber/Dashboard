...
def create_aligned_chart(data, x_axis_col, title):
    ...
    # PERBAIKAN: Fungsi helper format angka dipindah ke sini
    ...
    # PERBAIKAN: Tinggi grafik disesuaikan
...
with tab_screener:
    ...
    if not df.empty:
        ...
        if conditions:
            ...
            # Definisikan kolom untuk setiap tampilan
            ...
            if is_mobile:
                # Di mobile, tampilkan tabel simpel dulu
                st.dataframe(
                    result_df[display_cols].rename(columns=rename_cols).style.format(format_dict).background_gradient(cmap='Greens', subset=['Vol x MA20']),
                    use_container_width=True
                )
                # Sisanya di dalam expander
                with st.expander("Tampilkan Detail Tabel"):
                    st.dataframe(
                        result_df[desktop_cols].rename(columns=rename_cols).style.format(format_dict).background_gradient(cmap='Greens', subset=['Vol x MA20', 'Val x MA20']),
                        use_container_width=True
                    )
            else:
