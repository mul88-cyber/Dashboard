...
with tab_chart:
    ...
    if not df.empty:
        ...
        if not stock_data.empty:
            ...
            # PERBAIKAN: Ukuran font judul & subjudul diturunkan
        ...
        if selected_weeks:
            ...
            # PERBAIKAN: Ukuran font info minggu diturunkan
            st.caption(f"Menampilkan data untuk minggu: **{', '.join(sorted(selected_weeks))}**")
            create_aligned_chart(data=filtered_daily_data, x_axis_col='Last Trading Date', title="") # Judul di grafik dikosongkan
