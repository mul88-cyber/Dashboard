...
def create_aligned_chart(data, x_axis_col, title):
    ...
    fig.add_trace(go.Scatter(
        ...
        y=data['Close'],
        name='Harga',
        text=data['text_change'], # Sediakan teks yang akan ditampilkan
        textposition="bottom center", # Atur posisi teks
        textfont=dict(size=10, color='white'),
        # --- PERBAIKAN KUNCI: Tambahkan 'text' ke mode ---
        mode='lines+markers+text',
        hovertemplate=( # Hover tetap ada untuk info detail
            '<b>%{x|%d %b %Y}</b><br>'
            'Harga: %{y:,.0f}<br>'
