...
def create_aligned_chart(data, x_axis_col, title):
    ...
    fig.add_trace(go.Scatter(
        x=data[x_axis_col], y=data['Close'], name='Harga', text=data['text_change'], textposition="bottom center", textfont=dict(size=10, color='white'),
...
with tab_chart:
    ...
    if not df.empty:
        ...
        # --- PERBAIKAN: Logika Filter Cerdas ---
        # Inisialisasi session state untuk melacak saham terakhir yang dipilih
        ...
        if not stock_data.empty and 'Week' in stock_data.columns:
            ...
            selected_months = st.sidebar.multiselect("2. Pilih Bulan-Tahun", available_months, default=available_months[0] if available_months else None, key="selected_months")
